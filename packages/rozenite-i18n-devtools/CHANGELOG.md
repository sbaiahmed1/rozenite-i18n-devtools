...
### Performance
- Store-change snapshot pushes are now coalesced (default 400ms, `snapshotDebounceMs` to tune).
  Previously every i18next `added`/`removed`/`languageChanged` event triggered a full
